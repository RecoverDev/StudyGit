...
В **Git** каждый файл имеет один из четырех статусов

* **untracked** - не отслеживаемый. **Git** видит, что файл существует, но не следит за его изменениями.
* **staged** - подготовленный. После команды **git add** файл попадает в список изменений, которые войдут в следующий коммит.
* **tracked** - отслеживаемый. Этот статус получают все файлы после команды **git commit**, а также файлы после команды **git add**, то есть все файлы состояние которых **Git** так или иначе отслеживает.
* **modified** - измененный. Это состояние означает, что содержимое файла изменилось после последнего коммита.
